<!-- Dynamic Typing Banner -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&duration=3000&pause=500&color=00F7FF&center=true&vCenter=true&width=550&lines=Hi%2C+I'm+Rahul+Raj+%F0%9F%91%8B;MERN+Stack+Developer;Blockchain+Enthusiast;Building+Web3+DApps" alt="Typing SVG" />
</p>

<!-- Glowing Badges -->
<p align="center">
  <a href="https://github.com/rajlucifer">
    <img src="https://img.shields.io/badge/GitHub-rajlucifer-181717?style=for-the-badge&logo=github&logoColor=white&labelColor=0D1117&color=0D1117" />
  </a>
  <a href="https://linkedin.com/in/YourLinkedInProfile">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0A66C2&color=0A66C2" />
  </a>
  <a href="https://twitter.com/YourTwitterHandle">
    <img src="https://img.shields.io/badge/Twitter-Follow-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white&labelColor=1DA1F2&color=1DA1F2" />
  </a>
  <a href="https://your-portfolio-url.com">
    <img src="https://img.shields.io/badge/Portfolio-Visit-8A2BE2?style=for-the-badge&logo=vercel&logoColor=white&labelColor=8A2BE2&color=8A2BE2" />
  </a>
</p>

---

## 🚀 About Me

<p align="center">
  <img align="right" width="300" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" alt="Coding GIF" />
</p>

I'm a **MERN Stack Developer** & **Blockchain Enthusiast** – I build full‑stack web apps and decentralized voting systems on Ethereum.

- 🔭 Currently working on **Blockchain voting DApp** with Solidity & Web3.js
- 🌱 Learning **Smart Contract security** and **advanced React patterns**
- 👯 Open to collaborate on **MERN + Web3 projects**
- 💬 Ask me about: MERN, Solidity, Ethers.js, Web3.js
- 🎯 2026 goal: **Launch a production‑ready voting DApp** & contribute to open‑source
- ⚡ Fun fact: My first blockchain transaction was a testnet vote – now I build real voting systems 🗳️

<br clear="right"/>

---

## 🛠️ Tech Stack – MERN + Blockchain Tools

<p align="center">
  <!-- MERN core -->
  <img src="https://skillicons.dev/icons?i=mongodb,express,react,nodejs,js" />
  <br/>
  <!-- Blockchain tools -->
  <img src="https://skillicons.dev/icons?i=solidity" />
  <img src="https://img.shields.io/badge/Web3.js-F16822?style=for-the-badge&logo=web3.js&logoColor=white" alt="Web3.js" />
  <img src="https://img.shields.io/badge/Ethers.js-2535a0?style=for-the-badge&logo=ethereum&logoColor=white" alt="Ethers.js" />
  <br/>
  <!-- Frontend & tools -->
  <img src="https://skillicons.dev/icons?i=html,css,tailwind,git" />
</p>

<p align="center">
  <i>MERN for web apps + Solidity & Web3.js for blockchain voting.</i>
</p>

---

## 🔥 Featured Projects

| Project | Tech Stack | Key Features |
|---------|------------|---------------|
| 🔐 **Blockchain Voting System** | Solidity, Web3.js, React, Node.js, MongoDB | • MetaMask authentication<br>• Real‑time vote counting on‑chain<br>• Tamper‑proof audit trail<br>• Admin dashboard to add candidates |
| 🗳️ **Traditional Voting System** | MongoDB, Express, React, Node.js | • JWT authentication<br>• Email/SMS verification<br>• Live results dashboard |
| 📝 **Task Manager API** | Node.js, Express, MongoDB | • CRUD operations<br>• User roles & permissions<br>• RESTful architecture |
| 🛒 **E‑commerce Backend** | Node.js, Express, MongoDB | • Product & order management<br>• Payment gateway integration (Razorpay) |

> 🔗 All repositories are linked from my GitHub profile – click the project titles below.

<details>
<summary><b>📌 Click to see direct repo links</b></summary>

- [Blockchain Voting System](https://github.com/rajlucifer/Blockchain-voting-system)
- [Traditional Voting System](https://github.com/rajlucifer/voting-sytem)
- [Task Manager API](https://github.com/rajlucifer/task-manager)
- [E‑commerce Backend](https://github.com/rajlucifer/ecommerce-api)

</details>

---

## 📈 GitHub Stats – With Glow Effect

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=rajlucifer&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&title_color=00F7FF&icon_color=00F7FF" />
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rajlucifer&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&title_color=00F7FF" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=rajlucifer&theme=radical&hide_border=true&background=0D1117&stroke=00F7FF&ring=00F7FF&fire=00F7FF&currStreakNum=FFFFFF&sideNums=00F7FF" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=rajlucifer&theme=radical&no-frame=true&row=2&column=4&margin-w=10&margin-h=10" alt="GitHub Trophies" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=rajlucifer&label=Profile%20Views&color=00F7FF&style=flat-square" alt="Profile Views Counter" />
</p>

---

## 🌟 Cool Features You'll Find Here

- ✨ **Animated Typing Effect** – changes roles every 3 seconds
- 🎨 **Neon accent colors** (#00F7FF) across all badges & stats
- 🖼️ **Live Coding GIF** – adds personality
- 📁 **Collapsible Sections** – keeps the page clean
- 🔥 **Glowing Badges** with hover effect
- 📊 **All your stats in one place** – streaks, trophies, top languages

---

## 📫 Let's Connect

<p align="center">
  <a href="https://github.com/rajlucifer">
    <img src="https://img.shields.io/badge/GitHub-rajlucifer-181717?style=for-the-badge&logo=github&logoColor=white&labelColor=0D1117" />
  </a>
  <a href="mailto:rahul.raj@example.com">
    <img src="https://img.shields.io/badge/Email-rahul.raj@example.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/YourLinkedInProfile">
    <img src="https://img.shields.io/badge/LinkedIn-Rahul_Raj-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <i>⚡ “MERN + Blockchain = The future of transparent applications.”</i>
</p>
